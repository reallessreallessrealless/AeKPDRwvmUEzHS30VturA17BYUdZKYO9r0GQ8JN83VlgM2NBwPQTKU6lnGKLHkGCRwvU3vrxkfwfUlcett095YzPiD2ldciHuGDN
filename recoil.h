...
#pragma region StructRecoilBS
typedef float vec_t2;
struct vec2_t2
{
	vec_t2 x, y;
};
...
public:
	...
	vec3_t2() { x = y = z = 0; }
	vec3_t2(const vec_t2 x, const vec_t2 y, const vec_t2 z) : x(x), y(y), z(z) {}
	vec3_t2 operator + (const vec3_t2& rhs) const { return vec3_t2(x + rhs.x, y + rhs.y, z + rhs.z); }
	vec3_t2 operator - (const vec3_t2& rhs) const { return vec3_t2(x - rhs.x, y - rhs.y, z - rhs.z); }
	vec3_t2 operator * (const vec_t2& rhs) const { return vec3_t2(x * rhs, y * rhs, z * rhs); }
	vec3_t2 operator / (const vec_t2& rhs) const { return vec3_t2(x / rhs, y / rhs, z / rhs); }
	vec3_t2& operator += (const vec3_t2& rhs) { return *this = *this + rhs; }
	vec3_t2& operator -= (const vec3_t2& rhs) { return *this = *this - rhs; }
	vec3_t2& operator *= (const vec_t2& rhs) { return *this = *this * rhs; }
	vec3_t2& operator /= (const vec_t2& rhs) { return *this = *this / rhs; }
	vec_t2 dot() const { return x * x + y * y + z * z; }
	vec_t2 Length() const { return sqrtf(dot()); }
	vec3_t2 Normalize() const { return *this * (1 / Length()); }
	vec_t2 Distance(const vec3_t2& rhs) const { return (*this - rhs).Length(); }
};
...
enum targetAssistBehavior_t : int
{
...
enum AdsOffsetInterpolationType : int
{
...
struct AdvancedHipSwaySettings
{
	...
	vec2_t2 torsoGoalDeadzoneAdjustSpeed;
	RumbleGraph* torsoGoalViewSpeedToMaxDeadzone_graph;
	vec2_t2 torsoGoalViewSpeedToMaxDeadzone_viewspeed;
	vec2_t2 torsoGoalViewSpeedToMaxDeadzone_maxDeadzone;
	vec2_t2 torsoMass;
	vec2_t2 torsoSpring;
	vec2_t2 torsoDamper;
	int gunGoalViewSmoothDurationMs;
	RumbleGraph* gunGoalViewSpeedToOffset_graph;
	vec2_t2 gunGoalViewSpeedToOffset_viewspeed;
	vec2_t2 gunGoalViewSpeedToOffset_offset;
	vec2_t2 gunMass;
	vec2_t2 gunSpring;
	vec2_t2 gunDamper;
	vec3_t2 gunPivotPoint;
...
public:
	...
	vec3_t2 vStandOfs;
	vec3_t2 vStandOfsRot;
	vec3_t2 vStandOfsRotPivot;
	vec3_t2 vStandMove;
	vec3_t2 vStandRot;
	vec3_t2 strafeMove;
	vec3_t2 strafeRot;
	...
	vec3_t2 vDuckedOfs;
	vec3_t2 vDuckedOfsRot;
	vec3_t2 vDuckedOfsRotPivot;
	vec3_t2 vDuckedMove;
	vec3_t2 vDuckedRot;
	vec3_t2 vProneOfs;
	vec3_t2 vProneMove;
	...
	vec2_t2* originalAccuracyGraphKnots[2];
