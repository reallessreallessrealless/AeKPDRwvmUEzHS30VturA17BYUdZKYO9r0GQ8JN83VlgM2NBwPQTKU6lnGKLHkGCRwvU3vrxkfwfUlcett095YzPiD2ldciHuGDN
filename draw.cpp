...
namespace draw
{
	...
	{
		float x = *(float*)(*(uint64_t*)(var->baseModule + var->Offest.camera_base) + var->Offest.camera_view_x);
		float y = *(float*)(*(uint64_t*)(var->baseModule + var->Offest.camera_base) + var->Offest.camera_view_y);
		float z = *(float*)(*(uint64_t*)(var->baseModule + var->Offest.camera_base) + var->Offest.camera_view_z);
	...
	void draw_health(int i_health, Vector3 pos)
	{
		...
		auto color_health = var->Settings.Red;
		auto color_health1 = var->Settings.Green;
